...
def gender_predict(video):
    ...
    try:
        ...
        while True:
            ...
            # predict the faces
            faces = rf.detect_faces(frame)
            
            # Loop over the faces detected
            for key in faces.keys():
                identity = faces[key]
                face_area = identity["facial_area"]
                face_img = frame[face_area[2]: face_area[3], face_area[0]: face_area[1]]
                ...
                optimal_font_scale = get_optimal_font_scale(label,((face_area[1]-face_area[0])+25))
                box_color = (255, 0, 0) if gender == "Male" else (147, 20, 255)
                cv2.rectangle(frame, (face_area[0], face_area[2]), (face_area[1], face_area[3]), box_color, 2)
                
                # Label processed image
                cv2.putText(frame, label, (face_area[0], yPos),
